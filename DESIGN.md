...
Página de una sola sección larga (sin routing), contenida en un `.wrap` de ancho completo (`overflow: hidden`, necesario para recortar los acentos circulares de fondo que asoman por los bordes — ver abajo). Padding vertical de sección generoso en desktop (88-150px) que se comprime a 24px horizontal en mobile (`≤900px`, único breakpoint del sitio). Grillas de 3 columnas (pilares, registros) y 2 columnas (autoridades) colapsan a 1 columna en mobile. El header es sticky (`top: 0`) en todo momento, incluido el panel de navegación mobile que vive dentro del mismo elemento sticky. Las secciones siguen cortándose en ángulo recto entre sí — no hay curvas SVG ni transiciones onduladas.

**Fondo completo, contenido acotado (corregido 2026-08-27).** El sitio pasó por una primera versión donde `.wrap` tenía `max-width: 1440px`, lo que dejaba franjas del color de fondo base a los costados de las secciones con color propio (hero oscuro, SURi navy, footer) en monitores anchos (>1440px) — el sitio "no ocupaba toda la pantalla". Se corrigió así: las secciones con color de fondo distinto (`.hero`, `.suri`, `.contacto`, `footer.site-foot`) van a ancho completo, y una clase utilitaria `.contain` (`max-width: 1440px; margin: 0 auto`) se aplica solo al contenido que necesita un ancho de lectura acotado (barra del header, `.acciones`, `.autoridades`, `.registros`, `.foot-inner`). Nunca aplicar `max-width` directamente a un elemento que también lleve un `background-color`/`background-image` propio — eso reintroduce el mismo bug.
