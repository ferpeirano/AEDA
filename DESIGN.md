...
Plano por defecto. El sitio no usa sombra como recurso decorativo — aparece únicamente como respuesta a estado (hover) o para un elemento verdaderamente flotante (el sello de contacto).
...
Dos lenguajes de forma conviviendo a propósito: todo lo rectangular (botones, inputs, tarjetas, badges de registro) usa un radio chico y consistente de 3-4px — nunca `rounded-lg` genérico. Todo lo que sea retrato o marca (fotos de autoridades, íconos de pilares, isologo del hero/contacto) es un círculo perfecto (`border-radius: 50%`). No hay una forma intermedia; el círculo está reservado para "esto es una persona o una marca", el radio chico para "esto es un control o contenedor".

## Components

### Buttons
- **Shape:** radio de 3px, nunca completamente cuadrado ni pill (excepto el CTA "Contacto" del nav, que sí es un pill funcional por convención de nav).
...
El mismo patrón —un círculo con foto o logo dentro— se repite en cuatro lugares distintos del sitio: el isologo del header (46px), el isologo blanco del hero (56px de alto), las fotos de pilares y autoridades (140px / 58px), y el sello blanco con sombra de la sección de contacto (108px, el único con `box-shadow` en reposo). Es el elemento que más unifica visualmente el sitio de punta a punta.

## Do's and Don'ts

### Do:
- **Do** usar `var(--color-*)` para cualquier color nuevo — nunca un hex literal nuevo sin agregarlo primero al `:root`.
- **Do** mantener el círculo (`border-radius: 50%`) reservado para fotos y marcas; el radio de 3px para todo lo demás.
